...
<div class="mb-10 border-b-4 border-black pb-4 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
    <h2 class="text-4xl font-black uppercase tracking-tight">Pusat Komando</h2>
    <div class="flex space-x-4">
        <a href="{{ route('categories.index') }}" class="border-2 border-black bg-white px-6 py-2 text-sm font-bold uppercase tracking-widest hover:bg-black hover:text-white transition-colors shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1 hover:translate-y-1">Kategori</a>
        <a href="{{ route('cameras.index') }}" class="border-2 border-black bg-white px-6 py-2 text-sm font-bold uppercase tracking-widest hover:bg-black hover:text-white transition-colors shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1 hover:translate-y-1">Kamera</a>
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
    ...
    <div class="border-4 border-black bg-white p-6 relative shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all">
        ...
        <h3 class="text-sm font-bold uppercase tracking-widest text-gray-500 mb-2 border-b-2 border-black pb-2 inline-block">Total Kamera</h3>
    ...
    <div class="border-4 border-black bg-black text-white p-6 relative shadow-[8px_8px_0px_0px_rgba(0,0,0,0.2)] hover:translate-x-1 hover:translate-y-1 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)] transition-all">
        ...
        <h3 class="text-sm font-bold uppercase tracking-widest text-gray-400 mb-2 border-b-2 border-white pb-2 inline-block">Sewa Aktif</h3>
    ...
    <div class="border-4 border-black bg-yellow-300 p-6 relative shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all">
        <div class="absolute top-0 right-0 w-8 h-8 bg-black"></div>
        <h3 class="text-sm font-bold uppercase tracking-widest text-black mb-2 border-b-2 border-black pb-2 inline-block">Menunggu Verifikasi</h3>
    ...
    <div class="border-4 border-black bg-red-500 text-white p-6 relative shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all">
        <div class="absolute top-0 right-0 w-8 h-8 bg-black"></div>
        <h3 class="text-sm font-bold uppercase tracking-widest text-white mb-2 border-b-2 border-black pb-2 inline-block">Terlambat</h3>
        ...
        <div class="flex items-start">
            ...
            <div class="ml-6">
                <h3 class="text-2xl font-black uppercase tracking-tight mb-2">
                    Siaga: {{ $lateRentals->count() }} Sewa Terlambat!
                </h3>
                <div class="mt-4">
...
<div class="border-4 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] mb-12">
    <div class="border-b-4 border-black p-6 bg-gray-50 flex justify-between items-center">
        <h3 class="text-2xl font-black uppercase tracking-tight">Log Transaksi</h3>
    </div>
    <div class="overflow-x-auto p-6">
        <table class="w-full text-left border-collapse">
            <thead>
                <tr>
                    <th class="border-b-4 border-black pb-4 text-sm font-bold uppercase tracking-widest w-1/4">Pelanggan</th>
                    <th class="border-b-4 border-black pb-4 text-sm font-bold uppercase tracking-widest w-1/4">Detail Pesanan</th>
                    <th class="border-b-4 border-black pb-4 text-sm font-bold uppercase tracking-widest w-1/4">Status</th>
                    <th class="border-b-4 border-black pb-4 text-sm font-bold uppercase tracking-widest w-1/4 text-right">Tindakan</th>
                </tr>
            </thead>
                ...
                <tr class="hover:bg-gray-50 transition-colors group">
                    ...
                    <td class="py-6 px-4">
                        ...
                        <div class="text-sm font-bold flex gap-4 mt-2">
                            ...
                            <span>Sampai: {{ $rental->end_date->format('d/m/y') }}</span>
                        </div>
                    ...
                    <td class="py-6 pl-4 text-right">
                        @if($rental->payment && $rental->payment->status == 'pending')
                            <div class="flex flex-col items-end gap-2">
                                <a href="{{ asset('storage/' . $rental->payment->payment_proof) }}" target="_blank" class="border-2 border-black bg-white px-4 py-1 text-xs font-bold uppercase tracking-widest hover:bg-black hover:text-white transition-colors">Lihat Bukti</a>
                                
                                <div class="flex gap-2">
                                    <form action="{{ route('admin.payments.verify', $rental->payment->id) }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="status" value="verified">
                                        <button type="submit" class="border-2 border-black bg-black text-white px-4 py-1 text-xs font-bold uppercase tracking-widest hover:bg-green-500 transition-colors">Setuju</button>
                                    </form>
                                    <form action="{{ route('admin.payments.verify', $rental->payment->id) }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="status" value="rejected">
                                        <button type="submit" class="border-2 border-black bg-white text-black px-4 py-1 text-xs font-bold uppercase tracking-widest hover:bg-red-500 hover:text-white transition-colors">Tolak</button>
                                    </form>
                                </div>
                            </div>
                        @elseif($rental->status == 'approved')
                            <form action="{{ route('admin.rentals.return', $rental->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="border-2 border-black bg-black text-white px-6 py-3 text-xs font-bold uppercase tracking-widest hover:bg-white hover:text-black transition-colors shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1 hover:translate-y-1">Terima Pengembalian</button>
                            </form>
                        @else
                            <span class="text-xs font-bold uppercase tracking-widest text-gray-400">Tidak ada aksi</span>
                        @endif
                    </td>
                </tr>
                @empty
